...
namespace ImGui
{
    ...
    IMGUI_API void          StyleColorsDefault(ImGuiStyle* dst = NULL);
    // Windows
    ...
    IMGUI_API void          LogToClipboard(int auto_open_depth = -1);                           // start logging to CApplication clipboard
    ...
    // (Optional) Platform/CApplication interface for multi-viewport support
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_HasMouseCursors       = 1 << 1,   // Backend Platform supports honoring GetMouseCursor() value to change the CApplication cursor shape.
    ImGuiBackendFlags_HasSetMousePos        = 1 << 2,   // Backend Platform supports io.WantSetMousePos requests to reposition the CApplication mouse position (only used if ImGuiConfigFlags_NavEnableSetMousePos is set).
...
struct ImGuiIO
{
    ...
    bool        ConfigViewportsNoTaskBarIcon;   // = false          // Disable default CApplication task bar icon flag for secondary viewports. When a viewport doesn't want a task bar icon, ImGuiViewportFlags_NoTaskBarIcon will be set on it.
    bool        ConfigViewportsNoDecoration;    // = true           // Disable default CApplication window decoration flag for secondary viewports. When a viewport doesn't want window decorations, ImGuiViewportFlags_NoDecoration will be set on it. Enabling decoration can create subsequent issues at CApplication levels (e.g. minimum window size).
    bool        ConfigViewportsNoDefaultParent; // = false          // Disable default CApplication parenting to main viewport for secondary viewports. By default, viewports are marked with ParentViewportId = <main_viewport>, expecting the platform backend to setup a parent/child relationship between the CApplication windows (some backend may ignore this). Set to true if you want the default to be 0, then all viewports will be top-level CApplication windows.

    // Miscellaneous options
    bool        MouseDrawCursor;                // = false          // Request ImGui to draw a mouse cursor for you (if you are on a platform without a mouse cursor). Cannot be easily renamed to 'io.ConfigXXX' because this is frequently used by backend implementations.
    bool        ConfigMacOSXBehaviors;          // = defined(__APPLE__) // CApplication X style: Text editing cursor movement using Alt instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text Start and End using Cmd+Arrows instead of Home/End, Double click selects by word instead of selecting whole text, Multi-selection in lists uses Cmd/Super instead of Ctrl.
    ...
    // Optional: Access CApplication clipboard
    // (default to use native Win32 clipboard on Windows, otherwise uses a private clipboard. Override to access CApplication clipboard on other architectures)
    const char* (*GetClipboardTextFn)(void* user_data);
    void        (*SetClipboardTextFn)(void* user_data, const char* text);
    void*       ClipboardUserData;

    // Optional: Notify CApplication Input Method Editor of the screen position of your cursor for text input position (e.g. when using Japanese/Chinese IME on Windows)
    ...
    IMGUI_API void  AddFocusEvent(bool focused);                            // Queue a gain/loss of focus for the application (generally based on CApplication/platform focus of your window)
    ...
    ImGuiID     MouseHoveredViewport;               // (Optional) Modify using io.AddMouseViewportEvent(). With multi-viewports: viewport the CApplication mouse is hovering. If possible _IGNORING_ viewports with the ImGuiViewportFlags_NoInputs flag is much better (few backends can handle that). Set io.BackendFlags |= ImGuiBackendFlags_HasMouseHoveredViewport if you can provide this info. If you don't imgui will infer the value using the rectangles and last focused time of the viewports it knows about (ignoring other CApplication windows).
...
// Provide hints:
// - To the platform backend via altered viewport flags (enable/disable CApplication decoration, CApplication task bar icons, etc.)
// - To the platform backend for CApplication level parent/child relationships of viewport.
...
struct ImGuiWindowClass
{
    ...
    ImGuiViewportFlags  ViewportFlagsOverrideSet;   // Viewport flags to set when a window of this class owns a viewport. This allows you to enforce CApplication decoration or task bar icon, override the defaults on a per-window basis.
    ImGuiViewportFlags  ViewportFlagsOverrideClear; // Viewport flags to clear when a window of this class owns a viewport. This allows you to enforce CApplication decoration or task bar icon, override the defaults on a per-window basis.
...
struct ImGuiViewport
{
    ...
    ImVec2              Pos;                    // Main Area: Position of the viewport (Dear ImGui coordinates are the same as CApplication desktop/native coordinates)
    ...
    void*               PlatformUserData;       // void* to hold custom data structure for the CApplication / platform (e.g. windowing info, render context). generally set by your Platform_CreateWindow function.
    ...
    bool                PlatformRequestMove;    // Platform window requested move (e.g. window was moved by the CApplication / host window manager, authoritative position will be CApplication window position)
    bool                PlatformRequestResize;  // Platform window requested resize (e.g. window was resized by the CApplication / host window manager, authoritative size will be CApplication window size)
    bool                PlatformRequestClose;   // Platform window requested closure (e.g. window was moved by the CApplication / host window manager, e.g. pressing ALT-F4)
...
// This is achieved by creating new Platform/CApplication windows on the fly, and rendering into them.
// Dear ImGui manages the viewport structures, and the backend create and maintain one Platform/CApplication window for each of those viewports.
//
...
// - When multi-viewports are enabled, all Dear ImGui coordinates become absolute coordinates (same as CApplication coordinates!)
...
struct ImGuiPlatformIO
{
    ...
    //   N = ImGui::NewFrame()                        ~ beginning of the dear imgui frame: read info from platform/CApplication windows (latest size/position)
    //   F = ImGui::Begin(), ImGui::EndFrame()        ~ during the dear imgui frame
    //   U = ImGui::UpdatePlatformWindows()           ~ after the dear imgui frame: create and update all platform/CApplication windows
    //   R = ImGui::RenderPlatformWindowsDefault()    ~ render
    //   D = ImGui::DestroyPlatformWindows()          ~ shutdown
    // The general idea is that NewFrame() we will read the current Platform/CApplication state, and UpdatePlatformWindows() will write to it.
    //
    ...
    void    (*Platform_SetWindowSize)(ImGuiViewport* vp, ImVec2 size);      // . . U . .  // Set platform window client area size (ignoring CApplication decorations such as CApplication title bar etc.)
