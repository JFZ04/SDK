...
//  2023-02-15: Inputs: Use WM_NCMOUSEMOVE / WM_NCMOUSELEAVE to track mouse position over non-client area (e.g. CApplication decorations) when app is not focused. (#6045, #6162)
...
static bool ImGui_ImplWin32_UpdateMouseCursor()
{
    ...
    {
        // Hide CApplication mouse cursor if imgui is drawing it or if it wants no cursor
        ::SetCursor(nullptr);
    }
    else
    {
        // Show CApplication mouse cursor
...
// This code supports multi-viewports (multiple CApplication Windows mapped into different Dear ImGui viewports)
...
static void ImGui_ImplWin32_UpdateMouseData()
{
    ...
    if (is_app_focused)
    {
        // (Optional) Set CApplication mouse position from Dear ImGui if requested (rarely used, only when ImGuiConfigFlags_NavEnableSetMousePos is enabled by user)
        // When multi-viewports are enabled, all Dear ImGui positions are same as CApplication positions.
        ...
        // This also fills a short gap when clicking non-client area: WM_NCMOUSELEAVE -> modal CApplication move -> gap -> WM_NCMOUSEMOVE
        if (!io.WantSetMousePos && bd->MouseTrackedArea == 0 && has_mouse_screen_pos)
        {
            // Single viewport mode: mouse position in client window coordinates (io.MousePos is (0,0) when the mouse is on the upper-left corner of the app window)
            // (This is the position you can get with ::GetCursorPos() + ::ScreenToClient() or WM_MOUSEMOVE.)
            // Multi-viewport mode: mouse position in CApplication absolute coordinates (io.MousePos is (0,0) when the mouse is on the upper-left of the primary monitor)
    ...
    // (Optional) When using multiple viewports: call io.AddMouseViewportEvent() with the viewport the CApplication mouse cursor is hovering.
...
void    ImGui_ImplWin32_NewFrame()
{
    ...
    // Update CApplication mouse position
    ImGui_ImplWin32_UpdateMouseData();

    // Process workarounds for known Windows key handling issues
    ImGui_ImplWin32_ProcessKeyEventsWorkarounds();

    // Update CApplication mouse cursor with the cursor requested by imgui
