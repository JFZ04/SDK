...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (hovered && io.MouseClickedCount[0] >= 2 && !io.KeyShift)
        {
            ...
            if ((multiclick_count % 2) == 0)
            {
                ...
                // FIXME: There are likely many ways to improve this behavior, but there's no "right" behavior (depends on use-case, software, CApplication)
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_wordmove_key_down = is_osx ? io.KeyAlt : io.KeyCtrl;                     // CApplication X style: Text editing cursor movement using Alt instead of Ctrl
        const bool is_startend_key_down = is_osx && io.KeySuper && !io.KeyCtrl && !io.KeyAlt;  // CApplication X style: Line/Text Start and End using Cmd+Arrows instead of Home/End
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            // Notify CApplication of text input position for advanced IME (-1 x offset so that Windows IME can cover our cursor. Bit of an extra nicety.)
            if (!is_readonly)
            {
...
bool ImGui::BeginMainMenuBar()
{
    ...
    // FIXME: Consider removing support for safe area down the line... it's messy. Nowadays consoles have support for TV calibration in CApplication settings.
